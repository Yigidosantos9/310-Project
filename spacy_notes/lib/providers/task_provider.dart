...
class TaskController {
  ...
  await firestore.runTransaction((transaction) async {
    ...
    final previousCompletedBy = task.completedBy ?? '';
    if (previousCompletedBy.isNotEmpty && previousCompletedBy != currentUserId) {
      final previousUserRef = firestore.collection('users').doc(previousCompletedBy);
    ...
    if (currentUserSnap.exists) {
      ...
      currCompleted = newValue
          ? currCompleted + 1
          : (currCompleted > 0 ? currCompleted - 1 : 0);

      transaction.update(currentUserRef, {
        'tasksCompleted': currCompleted,
      });
    }
  });
  }
